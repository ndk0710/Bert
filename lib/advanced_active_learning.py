...
from lib.diversity_sampling import DiversitySampling
from lib.uncertainty_sampling import UncertaintySampling
from lib.pytorch_clusters import CosineClusters 
from lib.pytorch_clusters import Cluster
...
class AdvancedActiveLearning():
    ...
    def get_clustered_uncertainty_samples(self, model, unlabeled_data, device, cfg,
                                perc_uncertain = 0.1, num_clusters=20, max_epochs=10, limit=10000):
        """最も不確実性の高い項目を取得し、クラスタリングし、各クラスタからサンプリングする
        
        Keyword arguments:
            model -- 不確実性を判断するために予測を得るための機械学習モデル
            unlabeled_data -- まだラベルを持たないデータ
            method -- 不確実性サンプリング法 (eg: least_confidence())
            feature_method -- データから特徴を抽出する方法
            perc_uncertain -- 不確定性サンプリングによる項目のクラスター化率
            num_clusters -- 作成するクラスタの数
            max_epochs -- クラスターを作成する最大エポック数
            limit -- より高速なサンプリングのために、これだけの予測からサンプリングする (-1 = no limit)      
        """ 
        ...
        #不確実性サンプリング
        uncertain_samples = self.uncertainty_sampling.get_samples(model, unlabeled_data, device, cfg, uncertain_count, limit=limit)
        ...
        #空の要素削除
        samples = [row for row in samples if any(element for element in row)]

        for item in samples:
            item[2] = UncertaintySampling.least_confidence.__name__ + "_" + item[2]
