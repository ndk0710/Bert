...
class DiversitySampling():
    ...
    def get_cluster_samples(self, data, cfg, num_clusters=5, max_epochs=5, limit=5000):
        """余弦類似度を使用してクラスタを作成する
        
        キーワード引数:
            data -- クラスタリングするデータ
            cfg -- 特徴量取得用のトークナイザー情報
            num_clusters -- 作成するクラスタの数
            max_epochs -- クラスタを作成する最大エポック数
            limit -- クラスタリングを高速化するために、この数だけ項目をサンプリングする (-1 = 制限なし)
        
        K-Means クラスタリングアルゴリズムによってクラスタを作成、
        一般的なユークリッド距離の代わりに余弦類似度を使用
        
         収束するかmax_epochsがデータを通過するまでクラスタを作成
        ...
        cosine_clusters = CosineClusters(cfg, num_clusters)
